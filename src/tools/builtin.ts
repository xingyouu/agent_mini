 ...
 * 内置工具集
 *
 * Implementation note: src/tools/ 目录 (50+ 工具)
 *
...
export const execTool: Tool<{ command: string; timeout?: number }> = {
  ...
  async execute(input, ctx) {
    ...
    try {
      ...
      // AbortSignal → 杀进程
      // Implementation note: bash-tools.exec.ts — onAbortSignal → run.kill()
      const onAbort = () => {
       ...
       * 输出积累与截断
       *
       * Implementation note: bash-process-registry.ts → appendOutput()
 ...
 * 列出目录工具
 *
 * Implementation note: pi-coding-agent/core/tools/ls.ts
...
export const listTool: Tool<{ path?: string; limit?: number }> = {
  ...
  async execute(input, ctx) {
    ...
    try {
      ...
      // 按字母排序（大小写不敏感），对齐 mini-agent 的 ls 工具
 ...
 * 记忆写入工具
 *
 * Implementation note: 设计:
 * - Mini Agent 没有专用 memory_save 工具，LLM 用 write 工具写入 memory/YYYY-MM-DD.md
 * - mini 的 memory 系统是 JSON 索引（非文件系统），所以用专用工具替代
 * - 核心思想一致: LLM 自主决定什么值得记住，而非系统自动保存每轮对话
 *
 * Implementation note: src/auto-reply/reply/memory-flush.ts
